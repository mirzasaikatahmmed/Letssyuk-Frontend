import { type ReactNode } from "react";

interface VerificationStatsCardProps {
  title: string;
  value: number;
  icon: ReactNode;
  iconColor: "amber" | "cyan" | "red";
}

const iconRingColors = {
  amber: "border-amber-500/30 bg-amber-500/10 text-amber-400",
  cyan: "border-cyan-500/30 bg-cyan-500/10 text-cyan-400",
  red: "border-red-500/30 bg-red-500/10 text-red-400",
};

const VerificationStatsCard = ({
  title,
  value,
  icon,
  iconColor,
}: VerificationStatsCardProps) => {
  return (
    <div className="bg-[#0f1117] border border-white/[0.07] rounded-xl p-5 flex items-start justify-between flex-1 min-w-[160px]">
      <div className="flex flex-col gap-2">
        <span className="text-gray-400 text-sm">{title}</span>
        <span className="text-white text-3xl font-bold">{value}</span>
      </div>
      <span
        className={`w-10 h-10 rounded-full border flex items-center justify-center flex-shrink-0 ${iconRingColors[iconColor]}`}
      >
        {icon}
      </span>
    </div>
  );
};

export default VerificationStatsCard;
