...
const ConsentDeclarations = () => {
  ...
  return (
    <div className="p-4 text-white min-h-screen max-w-4xl mx-auto">
      <h2 className="text-xl font-bold mb-10 italic">Consent & Declarations</h2>
      ...
      >
        ...
        <div className="flex gap-3 bg-[#0d1e21] border border-cyan-900/30 p-4 rounded-xl items-start">
          ...
          <div>
            <p className="text-cyan-100/90 text-[11px] font-bold mb-1">
              Final Step: Consent & Declarations
            </p>
            <p className="text-cyan-100/60 text-[11px] leading-relaxed">
              Please review and confirm the following declarations to complete
              your profile.
            </p>
          </div>
        </div>
        ...
        <div className="space-y-6">
          {sections.map((section) => (
            <div
              key={section.id}
              className={`flex gap-4 p-6 rounded-2xl border transition-all ${
                watch(section.id)
            ...
            >
              <div className="mt-1">
                <Checkbox
                  id={section.id}
                  checked={watch(section.id)}
                  onCheckedChange={(checked) =>
                    setValue(section.id, !!checked, { shouldValidate: true })
                  }
        ...
        <div className="flex justify-between items-center mt-12 pt-4">
          <Button
            ...
            type="button"
            onClick={() => navigate(-1)}
            className="flex items-center gap-2 px-6 h-11 rounded-xl border-slate-800 text-gray-300 hover:bg-slate-800 hover:text-white transition-all text-sm font-medium"
          >
          ...
          <Button
            ...
            disabled={
              !watch("consentAI") ||
              !watch("gdprCompliance") ||
              !watch("roleUnderstanding")
            }
            className={`flex items-center gap-2 px-10 h-11 rounded-xl transition-all text-sm font-medium shadow-lg ${
              isValid
                ? "bg-[#234b52] hover:bg-[#2d5f68] text-cyan-100"
